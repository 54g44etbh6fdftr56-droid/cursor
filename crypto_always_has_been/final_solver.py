#!/usr/bin/python3
"""
Final solver - testing the most likely candidates based on the meme
"""
...
print("🎯 FINAL ATTEMPT - MEME ANALYSIS")
print("="*50)

# The meme format: "Wait, it's all X?" "Always has been"
# In crypto context: "Wait, it's all broken?" "Always has been"

# Most likely candidates based on the meme
meme_candidates = [
    # Direct meme reference
    b"HTB{1t_4lw4y5_h45_b33n}",
    b"HTB{it_always_has_been}",
    b"HTB{It_Always_Has_Been}",
    b"HTB{IT_ALWAYS_HAS_BEEN}",
    
    # With context about crypto being broken
    b"HTB{1t_4lw4y5_h45_b33n_br0k3n}",
    b"HTB{1t_4lw4y5_h45_b33n_w34k}",
    b"HTB{1t_4lw4y5_h45_b33n_b4d}",
    b"HTB{1t_4lw4y5_h45_b33n_34sy}",
    
    # Variations
    b"HTB{its_always_has_been}",
    b"HTB{it5_4lw4y5_h45_b33n}",
    b"HTB{!t_4lw4y5_h45_b33n}",
    b"HTB{it-always-has-been}",
    b"HTB{it.always.has.been}",
    b"HTB{it always has been}",
    
    # Without HTB wrapper
    b"1t_4lw4y5_h45_b33n",
    b"it_always_has_been",
    b"It_Always_Has_Been",
]

print(f"\n[*] Testing {len(meme_candidates)} meme-based candidates...")

for i, candidate in enumerate(meme_candidates):
    # Handle different lengths
    if len(candidate) == 32:
        # Perfect length - test as is
        ...
        pad_len = 32 - len(candidate)
        padded = candidate + bytes([pad_len]) * pad_len
        h = SecureHash(padded).digest()
        if h == target:
            print(f"\n✅ FLAG FOUND!")
            print(f"Flag: {candidate.decode()}")
            print(f"(Used PKCS7 padding: {pad_len} bytes)")
            exit(0)
        
        # Test with zero padding
        zero_padded = candidate + b'\x00' * (32 - len(candidate))
        h = SecureHash(zero_padded).digest()
        if h == target:
            print(f"\n✅ FLAG FOUND!")
            print(f"Flag: {candidate.decode()}")
            print(f"(Used zero padding)")
            exit(0)
    
    if (i + 1) % 5 == 0:
        print(f"[*] Tested {i + 1}/{len(meme_candidates)}...")

# Last resort - try the exact meme phrase
print("\n[*] Testing exact meme phrases...")

exact_meme = [
    b"Wait, it's all broken?Always has been",
    b"Wait its all broken Always has been",
    b"always has been",
    b"Always has been",
    b"ALWAYS HAS BEEN",
...
for phrase in exact_meme:
    if len(phrase) <= 32:
        # Test with padding to 32
        test = phrase + b'\x00' * (32 - len(phrase))
        h = SecureHash(test).digest()
        if h == target:
            print(f"\n✅ FOUND!")
            print(f"Phrase: {phrase.decode()}")
            print(f"Flag: HTB{{{phrase.decode()}}}")
            exit(0)

print("\n❌ No match found. The flag must be even more clever!")