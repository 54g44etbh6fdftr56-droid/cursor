#!/usr/bin/python3
"""
Testing without HTB{} wrapper - the flag might just be 32 raw bytes
"""
...
print("🔓 TESTING WITHOUT HTB{} WRAPPER")
print("="*50)

# The most obvious interpretations without wrapper
raw_flags = [
    # Direct meme reference - padded to 32 bytes
    b"1t_4lw4y5_h45_b33n______________",
    b"it_always_has_been______________",
    b"It_Always_Has_Been______________",
    b"always_has_been_________________",
    b"4lw4y5_h45_b33n_________________",
    b"Always_Has_Been_________________",
    b"ALWAYS_HAS_BEEN_________________",
    ...
    # Exact 32-char phrases
    b"it always has been, always will ",
    b"rolling your own crypto is easy!",
    b"never roll your own crypto ever!",
    b"homemade crypto always has been ",
    
    # Maybe it's about the description
    b"pretty easy crypto implementation",
    b"rolling crypto has been easy    ",
    
    # Common CTF patterns without wrapper
    b"th3_fl4g_1s_4lw4y5_h45_b33n_h3r3",
    b"y0u_f0und_1t_4lw4y5_h45_b33n_3z!",
]

print(f"[*] Testing {len(raw_flags)} raw candidates...")

for i, flag in enumerate(raw_flags):
    if len(flag) != 32:
        print(f"[-] Invalid length: {len(flag)} for {flag[:20]}...")
        continue
    ...
    if h == target:
        ...
        print(f"As string: {flag.decode('ascii', errors='replace')}")
        print(f"\nFor HTB format: HTB{{{flag.decode('ascii', errors='replace').strip('_.- !')}}}")
        exit(0)
    
    if (i + 1) % 10 == 0:
        print(f"[*] Tested {i + 1}/{len(raw_flags)}...")

# Test without any padding (exact phrases)
print("\n[*] Testing exact phrases...")

exact_phrases = [
    b"1t 4lw4y5 h45 b33n",
    b"it always has been",
    b"It Always Has Been", 
    b"always has been",
    b"4lw4y5 h45 b33n",
    b"Never roll your own",
    b"Pretty easy crypto",
]

for phrase in exact_phrases:
    # Pad with spaces to 32
    if len(phrase) < 32:
        test = phrase + b' ' * (32 - len(phrase))
        h = SecureHash(test).digest()
        if h == target:
            print(f"\n✅ FOUND: {phrase.decode()}")
            exit(0)
    
    # Pad with underscores to 32
    if len(phrase) < 32:
        test = phrase + b'_' * (32 - len(phrase))
        h = SecureHash(test).digest()
        if h == target:
            print(f"\n✅ FOUND: {phrase.decode()}")
            exit(0)

print("\n❌ Still no match. The flag is very well hidden!")